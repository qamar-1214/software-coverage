...
//     console.log("this is the sub cat", formData);
//     handleClose();
//   };
...
const CreateSubcategoryModal = ({
  ...
  const onSubmitForm = async (data) => {
    ...
    if (isEditing && !data.imageUrl && initialData?.imageUrl?.url) {
      formData.append("existingImageUrl", initialData.imageUrl.url);
    }

    console.log("FormData fields:", [...formData.entries()]); // Debug FormData
    try {
      await onSubmit(formData, isEditing, initialData?._id);
      handleClose();
    } catch (error) {
